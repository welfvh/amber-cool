#!/bin/bash
# amber-temp daemon installer. Installs fanctl + a root LaunchDaemon that holds
...
PLIST_SRC="$SCRIPT_DIR/co.welf.amber-temp.plist"
...
install -d -m 755 /usr/local/bin /usr/local/etc/amber-temp /usr/local/var/log
...
launchctl enable system/co.welf.amber-temp 2>/dev/null || true

sleep 3
echo "==> Status:"
/usr/local/bin/fanctl read || true
echo
echo "Done. amber-temp is running in mode '$MODE'."
echo "Change mode any time:  echo 'scale 7' | sudo tee /usr/local/etc/amber-temp/mode"
