# amber-cool — Research: How to control Mac fans properly
...
- **Coexistence caveat:** Macs Fan Control (and `FanBar.app`) are installed. Two apps writing SMC fan keys will fight. During amber-cool development, set Macs Fan Control to Auto or quit it.
...
amber-cool.app/Contents/
  MacOS/amber-cool                              (menu bar app, unprivileged)
...
- Daemon validates every incoming connection with `connection.setCodeSigningRequirement(...)` (macOS 13+) using a requirement string pinning **same Developer ID team**:
  `identifier "co.welf.amber-cool" and anchor apple generic and certificate leaf[subject.OU] = "6Y24LA63S7"`
...
- **`ProducerGuy/ThermalForge`** — open-source M1–M5 menu-bar + CLI fan control (does the handshake + watchdog). Closest end-to-end analog to amber-cool.
...
- **Coexistence** — Macs Fan Control + FanBar are installed and will fight amber-cool over SMC. Quit/disable them while developing and testing.
